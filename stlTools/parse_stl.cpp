...
namespace stl {
  ...
  stl_data parse_stl(const std::string& stl_path) {
    ...
    //Pass the string around to cast as unsigned int
    unsigned int* r = (unsigned int*) n_triangles;
    unsigned int num_triangles = *r;
    
    for (unsigned int i = 0; i < num_triangles; i++) {
      auto normal = parse_point(stl_file);
      auto v1 = parse_point(stl_file); //take 16bytes
      auto v2 = parse_point(stl_file);
      auto v3 = parse_point(stl_file);
      
      //Add the new triangle to the end of <vector> info
      info.triangles.push_back(triangle(normal, v1, v2, v3));
      
      //Some remnant data at the end of file
      char dummy[2];
